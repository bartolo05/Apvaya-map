...
async function fantasyMap(
  ...
  L.Projection.hex = L.extend({}, L.Projection.LongLat, {
    project: function (latlng) {
      return L.point(
        latlng.lng / mapDim.pixelsPerTile,
        latlng.lat / mapDim.pixelsPerTile
      );
    },

    unproject: function (point) {
      return new L.latLng(
        point.y * mapDim.pixelsPerTile,
        point.x * mapDim.pixelsPerTile
      );
    },
  ...
  L.CRS.pr = L.extend({}, L.CRS.Simple, {
    ...
    transformation: new L.Transformation(
      mapDim.factorx,
      mapDim.referencesize / mapDim.pixelsPerTile / 2,
      -mapDim.factory,
      mapDim.referencesize / mapDim.pixelsPerTile / 2
    ),
  ...
  var mapCenter = map.unproject(
    [mapDim.referencesize / 2, mapDim.referencesize / 2],
    8
  );
...
function pointToLayer(feature, latlng) {
  ...
  var customIcon = L.icon({
    ...
    shadowUrl:
      "https://pathfinder-gm-tools.s3.amazonaws.com/map/MerisylMapLocationMarkerShadow.png",
...
function locationFinder(map) {
  ...
  marker.on("dragend", function (e) {
    marker
      .getPopup()
      .setContent(JSON.stringify(marker.toGeoJSON()))
      .openOn(map);
  });
}
...
function displayGroup(key, clusterGroup, layerDisplayGroups) {
  ...
  if (layerDisplayGroups.hasOwnProperty(key)) {
    ...
    ldg = L.featureGroup.subGroup(clusterGroup, null);
...
function organizeClusters(map, layerDisplayGroups) {
  ...
  var layerControl = L.control
    .layers(null, layerDisplayGroups, {
      collapsed: false,
      sortLayers: true,
    })
    .addTo(map);
    ...
    _clicked: function (e) {
      ...
      if (
        ...
        if (this._clickCount > 1) {
          displayDistance = this._totalLength.toFixed(
            this.options.lengthUnit.decimal
          );
        } else {
          displayDistance = this._result.Distance.toFixed(
            this.options.lengthUnit.decimal
          );
        }
